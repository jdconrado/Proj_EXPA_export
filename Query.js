        ...
        apps.data.forEach(app => {
          try {
            matrix.push([
              ...
              app.opportunity.earliest_start_date
                ? String(app.opportunity.earliest_start_date).split("T")[0]
                : "",
              app.experience_end_date
                ? String(app.experience_end_date).split("T")[0]
                : "",
              ...
              app.opportunity.sub_product
                ? app.opportunity.sub_product.name
                : "-",
  ...
  try {
    ...
    if (opps !== undefined && opps.paging.total_pages <= 100) {
      ...
      do {
        ...
        page++;
        if (page <= totalP) {
          opps = await QueryCtrl.makeOppReq(
            QueryCtrl.formOppReq(req, page, 100)
          );
        }
      } while (page <= totalP);
  ...
  try {
    ...
    if (SnS !== undefined && SnS.paging.total_pages <= 100) {
      ...
      do {
            ...
            app.standards.forEach(standard => {
              row.push(
                standard.standard_option
                  ? standard.standard_option.option
                  : "Not answered"
              );
            });
        ...
        page++;
        if (page <= totalP) {
          SnS = await QueryCtrl.makeReq(
            QueryCtrl.formStandardsReq(req, page, 100)
          );
        }
      } while (page <= totalP);
  ...
  await transporter.sendMail(
    {
      ...
      subject: `[IMP] ${date.toISOString()} Your request have been processed: Link to the extracted ${
        req.body.type
      }`, // Subject line
